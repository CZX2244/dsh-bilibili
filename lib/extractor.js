...
/** 视觉模型两档：档位关键词 → 模型名（Ollama tag / llama-server alias；低/中 适配不同配置；高档已移除，更大模型用显式名）。 */
const VISION_TIERS = {
  low: "qwen3-vl:2b",
  small: "qwen3-vl:2b",
  medium: "qwen3-vl:8b",
...
/** Retired tier keywords fall back to the medium model instead of leaking as invalid names. */
const RETIRED_VISION_TIERS = new Set(["high", "large"]);
...
export function resolveVisionModel(visionModel) {
  ...
  if (RETIRED_VISION_TIERS.has(lower)) return VISION_TIERS.medium;
  return VISION_TIERS[lower] ?? model;
}
...
/** Short prompt for small/low-tier models (long instructions hurt instruction-following at 2B scale). */
export const VISION_PROMPT_SHORT =
  "用中文简述这张视频截图的内容：画面里在发生什么、展示了什么。2 句话以内，只描述事实。" +
  "若画面只是主持人口播，回复「纯口播画面」。" +
...
export function resolveVisionPrompt(model, userPrompt, userPromptByModel) {
  ...
  const tier = lower === "low" || lower === "small"
    ...
    : lower === "high" || lower === "large"
      ? "high"
      : /2b|small|tiny/.test(lower)
        ? "low"
        : /32b|72b|235b|large/.test(lower)
          ? "high"
          : "medium";
